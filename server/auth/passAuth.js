...
let localLogin = new LocalStrategy(options, async (email, password, done)=>{ // generally, username is the first in the parenthesis where email is. so use option to remap
    ...
    try{
        ...
        if(records !== null){
            ...
            bcrypt.compare(password, records[0].password, (err, isMatch)=>{ // compares the password input by user to the record in the db object
