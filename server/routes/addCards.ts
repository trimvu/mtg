...
router.post("/cardList", async(req: Request, res: Response) => {
    let listID = req.body;
...
router.get("/cardAll", async(req: Request, res: Response) => {

    try {
        const allCards = await db.cards.findAll()
        // console.log("all list cards ", allCards.sort((a, b) => a.id - b.id))
        res.json(allCards.sort((a: AllCardsProps, b: AllCardsProps) => a.id - b.id))
    } catch (err) {
        return res.status(624).json({error: "Can't find database //get all cards from a list"})
    }
})
...
router.put("/card/:id", async (req: Request, res: Response) => {
...
router.put("/card-price-update/:id", async (req: Request, res: Response) => {
...
router.delete("/card/:id", async (req: Request, res: Response) => {
    // let id = req.body;
...
router.delete("/deleteAllCards/:listID", async (req: Request, res: Response) => {
    // let id = req.body;
